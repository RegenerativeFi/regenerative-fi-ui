...
// );
const opportunitiesApr = computed(
  (): AprBreakdown['opportunitiesAprs'] | undefined =>
    apr.value?.opportunitiesAprs
);
...
// const opportunitiesAprRange = computed(() => {
...
<template>
  <div v-if="(opportunitiesApr?.total || 0) > 0" data-testid="staking-apr">
    <BalHStack justify="between" class="font-bold">
      <span>Merkl rewards</span>
      {{ fNum(opportunitiesApr?.total || 0, FNumFormats.bp) }}
    </BalHStack>
